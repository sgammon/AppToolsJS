...
class $
    ...
    get: (query, node=document) -> # ID, class or tag
        ...
        if (parts = query.split('.')).length > 1
            ...
            return (if result.length > 0 then $::to_array(result) else [])
    ...
    add_class: (element, cls) ->
        (if element.classList? then element.classList.add(cls) else element.className += ' '+cls)
        return element

    remove_class: (element, cls) ->
        (if element.classList? then element.classList.remove(cls) else element.className.replace(new RegExp('\\s*'+cls+'\\s*'), ' '))
        return element
...
HTMLElement.prototype.find = (query) -> return _.get(query, @)
HTMLElement.prototype.remove = () -> return _.remove(@)
HTMLElement.prototype.getOffset = () -> return _.get_offset(@)
HTMLElement.prototype.hasClass = (cls) -> return _.has_class(@, cls)
HTMLElement.prototype.addClass = (cls) -> return _.add_class(@, cls)
HTMLElement.prototype.removeClass = (cls) -> return _.remove_class(@, cls)
HTMLElement.prototype.isChild = (parent) -> return _.is_child(parent, @)
HTMLElement.prototype.isParent = (child) -> return _.is_child(@, child)
